...
# Entferne seq_len Parameter
def train_online(epochs=5, lr=1e-3, batch_size=1, log_csv="train_val_log.csv"):
    ...
    for ep in range(1, epochs+1):
        ...
        if val_rmse < best_val_loss:
            ...
            print(f"  Neuer Best-Checkpoint (val loss={val_rmse:.6f}) gespeichert")

# Test: Schritt-für-Schritt Streaming-Inferenz ohne Fensterpuffer
def test_online(log_csv="train_val_log.csv"):
    ...
    h_val, c_val = init_hidden(model, batch_size=1, device=device)
    preds_val, gts_val = [], []
    for v, i, true in tqdm(zip(df_val['Voltage[V]'], df_val['Current[A]'], df_val['SOC_ZHU']),
        ...
        preds_val.append(pred.item()); gts_val.append(true)
    ...
    # Streaming-Test auf Test
    h, c = init_hidden(model, batch_size=1, device=device)
    preds, gts = [], []
    for v,i in tqdm(zip(df_test['Voltage[V]'],df_test['Current[A]']),
                     total=len(df_test),desc="Testing Test"):
        x = torch.tensor([[v,i]],dtype=torch.float32,device=device).view(1,1,2)
        pred,(h,c)=model(x,(h,c)); preds.append(pred.item()); gts.append(df_test['SOC_ZHU'].iloc[len(preds)-1])
    mae = np.mean(np.abs(np.array(preds)-np.array(gts)))
    rmse = np.sqrt(np.mean((np.array(preds)-np.array(gts))**2))
    print(f"Test Stream - MAE: {mae:.4f}, RMSE: {rmse:.4f}")
