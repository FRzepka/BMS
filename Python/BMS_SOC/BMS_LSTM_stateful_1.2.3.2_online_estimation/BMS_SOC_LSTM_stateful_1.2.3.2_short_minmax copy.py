...
def train_online(epochs=5, lr=1e-3, batch_size=128, online_train=False, log_csv_path="training_log.csv"):
    ...
    for name, df in train_scaled.items():
        ...
        plt.subplot(3, 1, 1)
        plt.plot(df['timestamp'], df['SOC_ZHU'], 'b-', label="SOC")
        ...
        # Voltage Plot
        plt.subplot(3, 1, 2)
        plt.plot(df['timestamp'], df['Voltage[V]'], 'g-', label="Voltage [scaled]")
        plt.ylabel("Voltage (scaled)")
        plt.legend()
        
        # Current Plot
        plt.subplot(3, 1, 3)
        plt.plot(df['timestamp'], df['Current[A]'], 'r-', label="Current [scaled]")
        plt.xlabel("Zeit")
    ...
    plt.figure(figsize=(12,8))
    plt.subplot(3, 1, 1)
    plt.plot(df_val['timestamp'], df_val['SOC_ZHU'], 'b-', label="SOC")
    plt.title(f"Validation Data - {len(df_val)} samples")
    plt.ylabel("SOC")
    plt.legend()
    
    plt.subplot(3, 1, 2)
    plt.plot(df_val['timestamp'], df_val['Voltage[V]'], 'g-', label="Voltage [scaled]")
    plt.ylabel("Voltage (scaled)")
    plt.legend()
    
    plt.subplot(3, 1, 3)
    plt.plot(df_val['timestamp'], df_val['Current[A]'], 'r-', label="Current [scaled]")
    plt.xlabel("Zeit")
    plt.ylabel("Current (scaled)")
    plt.legend()
    ...
    plt.figure(figsize=(12,8))
    plt.subplot(3, 1, 1)
    plt.plot(df_test['timestamp'], df_test['SOC_ZHU'], 'b-', label="SOC")
    plt.title(f"Test Data - {len(df_test)} samples")
    plt.ylabel("SOC")
    plt.legend()
    
    plt.subplot(3, 1, 2)
    plt.plot(df_test['timestamp'], df_test['Voltage[V]'], 'g-', label="Voltage [scaled]")
    plt.ylabel("Voltage (scaled)")
    plt.legend()
    
    plt.subplot(3, 1, 3)
    plt.plot(df_test['timestamp'], df_test['Current[A]'], 'r-', label="Current [scaled]")
    plt.xlabel("Zeit")
    plt.ylabel("Current (scaled)")
    plt.legend()
        ...
        for name, df in train_scaled.items():
            if not online_train:
                ...
                for x_b, y_b in tqdm(dl, desc=f"{name} Ep{ep}", leave=True):
                    ...
                    # jedes Batch mit korrekter batch_size initialisieren
