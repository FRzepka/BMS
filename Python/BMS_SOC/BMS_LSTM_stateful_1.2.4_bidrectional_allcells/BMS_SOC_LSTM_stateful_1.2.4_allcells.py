...
DEFAULT_LR = 1e-4
SEQ_CHUNK_SIZE = 2048   # Original-Chunk-Größe
...
def load_data(base_path: str = "/home/florianr/MG_Farm/5_Data/MGFarm_18650_Dataframes"):
    ...
    # Lade Trainings-DFs mit Timestamp
    train_dfs = {}
    ...
    # Skalar mit Sampling pro Zelle fitten
    ...
    # Skaliere und behalte nur benötigte Spalten
    train_scaled = {}
    ...
    L = len(df3)
    i_val, i_test = int(L*0.5), int(L*0.7)
...
def train_online(epochs=30, online_train=False, log_csv_path="training_log.csv"):
    ...
    plt.title(f"Val SOC ({val_cell})")
    plt.tight_layout(); plt.savefig(f"val_data_plot_{val_cell}.png"); plt.close()
    plt.figure(figsize=(8,4))
    plt.plot(df_test['timestamp'], df_test['SOC_ZHU'])
    plt.title(f"Test SOC ({val_cell})")
    plt.tight_layout(); plt.savefig(f"test_data_plot_{val_cell}.png"); plt.close()
    ...
    for ep in range(1, epochs+1):
        ...
        # Seq2Seq-Validation über *einen* Chunk (Val-Set)
...
def test_seq2seq(log_csv_path="training_log.csv"):
    ...
    # Seq2Seq-Test über *einen* Chunk (Test-Set)
