...
def evaluate_seq2seq(model, df, device):
    ...
    with torch.no_grad():
        for i in range(n_chunks):
            ...
            chunk = torch.tensor(seq[s:e], dtype=torch.float32, device=device).unsqueeze(0).contiguous()  # Shape now (1, N, 4)
            model.lstm.flatten_parameters()
...
def train_online(epochs=30, lr=1e-4, online_train=False,
    ...
    for ep in range(1, epochs+1):
        ...
        for name, df in train_scaled.items():
            if not online_train:
                ...
                dl = DataLoader(
                    ...
                    batch_size=1,
                    shuffle=False,
                    num_workers=0,       # nur Hauptprozess
                    pin_memory=False     # kein gepinntes Memory
                )
                h, c = init_hidden(model, device=device)
        ...
        # Speicher freigeben, um OOM-Kill zu verhindern
...
def test_seq2seq(log_csv_path="training_log.csv"):
    ...
    ).unsqueeze(0).contiguous()  # Shape now (1, L, 4)
...
def objective(trial):
    ...
    lr  = trial.suggest_float("lr", 1e-5, 1e-3, log=True)  # neuer Optuna-API-Aufruf
    # Train & Val
