...
def train_online(epochs=20, lr=1e-3):
    ...
    # Training pro Zelle mit Reset des Hidden-States
    for ep in range(1, epochs+1):
        model.train()
        total_loss, total_steps = 0.0, 0
        print(f"\n=== Epoch {ep}/{epochs} ===")
        for name, df in train_scaled.items():
            print(f"--> Training auf Zelle {name} ({len(df)} Schritte)")
            ds = OnlineDataset(df)
            dl = DataLoader(ds, batch_size=1, shuffle=False)
            # Reset hidden state vor neuer Zelle
            h = torch.zeros(model.lstm.num_layers, 1, model.lstm.hidden_size, device=device)
            c = torch.zeros_like(h)
            for x_t, y_t in tqdm(dl, desc=f"{name} Ep{ep}", leave=False):
...
# Test: Schritt für Schritt über separate Zellen (reset pro Zelle)
def test_online():
    train_scaled, df_val, df_test, train_cells, val_cell = load_data()
    ...
    c = torch.zeros_like(h)
    preds, gts, ts = [], [], []
    ds_test = OnlineDataset(df_test)
    for x_t, y_t in tqdm(DataLoader(ds_test, batch_size=1, shuffle=False), desc="Testing"):
        x = x_t.unsqueeze(0).to(device)
        pred, (h, c) = model(x, (h, c))
        preds.append(pred.item()); gts.append(y_t.item()); ts.append(None)
