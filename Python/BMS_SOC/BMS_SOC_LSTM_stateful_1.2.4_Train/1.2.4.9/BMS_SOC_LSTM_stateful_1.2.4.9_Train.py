...
# Konstanten
SEQ_CHUNK_SIZE      = 4000    # Länge der Zeitreihen-Chunks (Standard)
USE_FULL_SEQUENCE   = False   # True → pro Zelle nur 1 Batch mit voller Länge
HIDDEN_SIZE         = 128
MLP_HIDDEN          = 128
...
def train_online(
    ...
    for name, dfv in df_vals.items():
        ...
        plt.legend(loc="upper right"); plt.grid(2)
...
if __name__ == "__main__":
    if train_scaled_glob is not None and df_vals_glob is not None and feature_scaler_glob is not None and train_cells_glob and val_cells_glob:
        ...
        if not train_scaled_glob:
            ...
            train_online(
                epochs=500, lr=1e-3, # Beispielwerte
                dropout=0.01, patience=300, # Beispielwerte
