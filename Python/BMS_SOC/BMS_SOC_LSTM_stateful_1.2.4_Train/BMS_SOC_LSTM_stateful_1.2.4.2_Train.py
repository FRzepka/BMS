...
# Konstanten - VERBESSERTE ARCHITEKTUR
SEQ_CHUNK_SIZE = 4096    # Länge der Zeitreihen-Chunks für Seq-to-Seq
HIDDEN_SIZE = 64         # ERHÖHT von 32 auf 64 für bessere Kapazität
MLP_HIDDEN = 64          # ERHÖHT für bessere Kapazität
NUM_LAYERS = 2           # ERHÖHT von 1 auf 2 LSTM Layers
...
def load_data_memory_efficient(base_path: str = "/home/florianr/MG_Farm/5_Data/MGFarm_18650_Dataframes"):
    ...
    # ERWEITERTE Zellaufteilung für mehr Trainingsdaten
    train_cells = [
        "MGFarm_18650_C01",
        "MGFarm_18650_C03", 
        "MGFarm_18650_C05",
        ...
        "MGFarm_18650_C17",
        "MGFarm_18650_C23",
        # ZUSÄTZLICHE Trainingszellen für bessere Generalisierung
        "MGFarm_18650_C09",
        "MGFarm_18650_C13",
        "MGFarm_18650_C15"
    ]
...
def train_model(model, train_scaled, val_dfs, train_cells, val_cells, out_dir, epochs=50):
    ...
    initial_lr = 3e-4  # Höhere initiale Lernrate
    optim = torch.optim.AdamW(model.parameters(), lr=initial_lr, weight_decay=5e-5, eps=1e-8)
    
    # VERBESSERTE Learning Rate Scheduler - Kombination von beiden
    scheduler1 = CosineAnnealingWarmRestarts(optim, T_0=10, T_mult=2, eta_min=1e-6)
    scheduler2 = ReduceLROnPlateau(optim, mode='min', factor=0.7, patience=5, min_lr=1e-6)
    
    criterion = nn.MSELoss()
    
    # Mixed Precision aktiviert für CUDA
    use_amp = (device.type == 'cuda')
    scaler_amp = GradScaler(enabled=use_amp)
    
    best_val_rmse = float('inf')
    no_improve = 0
    patience = 15  # Erhöhte Patience
    ...
    for ep in range(1, epochs+1):
        ...
        for name, df in train_scaled.items():
            ...
            for x_b, y_b in tqdm(dl, desc=f"{name} Ep{ep}", leave=False):
                ...
                # VERBESSERTE Gradient clipping
                clip_grad_norm_(model.parameters(), max_norm=2.0)  # Erhöht von 1.0
        ...
        # VERBESSERTE Learning Rate Scheduling
        scheduler1.step()  # CosineAnnealingWarmRestarts
        scheduler2.step(avg_val_rmse)  # ReduceLROnPlateau
