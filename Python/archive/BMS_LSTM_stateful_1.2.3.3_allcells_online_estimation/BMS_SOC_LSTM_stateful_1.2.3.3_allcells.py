...
def load_cell_data(data_dir: Path):
    needed = ["Absolute_Time[yyyy-mm-dd hh:mm:ss]",
              "Voltage[V]", "Current[A]", "SOH_ZHU", "SOC_ZHU"]
    dataframes = {}
...
def load_data(base_path: str = "/home/florianr/MG_Farm/5_Data/MGFarm_18650_Dataframes"):
    ...
    print(f"DEBUG: found cells keys = {list(cells.keys())}")
    cells.pop("MGFarm_18650_C29", None)                       # exclude C29 entirely
    names = sorted(cells.keys())
    val_cell = "MGFarm_18650_C17"
    train_cells = [n for n in names if n not in (val_cell, "MGFarm_18650_C29")]
    print(f"DEBUG: train_cells = {train_cells}, val_cell = {val_cell}")

    feats = ["Voltage[V]", "Current[A]", "SOH_ZHU"]          # include SOH_ZHU
    ...
    for name in train_cells:
        ...
        train_dfs[name] = df
        print(f"DEBUG: loaded df for {name}, shape = {df.shape}")

    # Skalar fitten mit Sampling pro Zelle
    MAX_SAMPLES_PER_CELL = 1000000
    sample_list = []
    for name, df in train_dfs.items():
        n = min(len(df), MAX_SAMPLES_PER_CELL)
        sample_list.append(df[feats].sample(n=n, random_state=42))
        print(f"DEBUG: sampled {n} rows from {name} for scaler fitting")
    df_all_train = pd.concat(sample_list, ignore_index=True)
    print(f"DEBUG: df_all_train sampled shape = {df_all_train.shape}")
    scaler = MaxAbsScaler().fit(df_all_train)
    print("DEBUG: scaler fit complete")

    # nur benötigte Spalten kopieren und skalieren
    train_scaled = {}
    for name, df in train_dfs.items():
        df2 = df[feats + ["SOC_ZHU"]].copy()
        df2[feats] = scaler.transform(df2[feats])
        train_scaled[name] = df2
        print(f"DEBUG: train_scaled[{name}] shape = {df2.shape}")

    # Urspr. DataFrames freigeben (nur train_dfs, cells wird weiter unten noch verwendet)
    del train_dfs
    gc.collect()
    ...
    L = len(df3)
    i_val = int(L * 0.5)                                      # 50% für Validierung
    i_test = i_val + int(L * 0.2)                             # nächste 20% für Test
    print(f"DEBUG: df3 length = {L}, split indices = ({i_val}, {i_test})")
    df_val = df3.iloc[:i_val].copy()
    df_test = df3.iloc[i_val:i_test].copy()
    print(f"DEBUG: df_val shape = {df_val.shape}, df_test shape = {df_test.shape}")
    df_val[feats] = scaler.transform(df_val[feats])
    df_test[feats] = scaler.transform(df_test[feats])
    print(f"DEBUG: df_val[feats] head =\n{df_val[feats].head()}")
    print(f"DEBUG: df_test[feats] head =\n{df_test[feats].head()}")
...
def evaluate_seq2seq(model, df, device):
    ...
    """
    model.eval()
    seq    = df[["Voltage[V]", "Current[A]", "SOH_ZHU"]].values
...
def evaluate_online(model, df, device):
    ...
    with torch.no_grad():
        for idx, (v, i, soh) in enumerate(
            tqdm(zip(df['Voltage[V]'].values, df['Current[A]'].values, df['SOH_ZHU']),
                 total=len(df), desc="Validation", leave=False)
        ):
            x = torch.tensor([[v, i, soh]], dtype=torch.float32, device=device).view(1,1,3)
...
def train_online(epochs=30, lr=1e-4, online_train=False, log_csv_path="training_log.csv"):
    ...
    # [Initial train/val/test plotting removed]
    ...
    for ep in range(1, epochs+1):
        ...
        for name, df in train_scaled.items():
            if not online_train:
                ...
                for idx, (v, i, y_true, soh) in enumerate(
                    tqdm(zip(df['Voltage[V]'].values, df['Current[A]'].values, df['SOC_ZHU'].values, df['SOH_ZHU'].values),
                         total=len(df), desc=f"{name} Ep{ep}", leave=True)):
                    x = torch.tensor([[v, i, soh]], dtype=torch.float32, device=device).view(1,1,3).contiguous()
                    y = torch.tensor([[y_true]], dtype=torch.float32).contiguous()
