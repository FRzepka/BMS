...
def load_data(base_path: str = "/home/florianr/MG_Farm/5_Data/MGFarm_18650_Dataframes"):
    ...
    for name, df in train_dfs.items():
        ...
        print(f"DEBUG: train_scaled[{name}] shape = {df2.shape}, first scaled row = {df2.iloc[0][feats].values}")
    ...
    print(f"DEBUG: df3 length = {L}, split indices = ({i1}, {i2})")
        ...
        for name, df in train_scaled.items():
            if not online_train:
                ...
                for idx, (v, i, y_true) in enumerate(
                    ...
                    x = torch.tensor([[v, i]], dtype=torch.float32, device=device).view(1,1,2).contiguous()
...
def test_seq2seq(log_csv_path="training_log.csv"):
    ...
    model.lstm.flatten_parameters()
    feats = torch.tensor(
        df_test[["Voltage[V]", "Current[A]"]].values,
