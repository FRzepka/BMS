...
# Windowed Dataset für schnelles Training/Validation
...
# Evaluations-Funktion für Validation
def evaluate(model, df, seq_len=50, batch_size=64):
    model.eval()
    ...
    with torch.no_grad():
        for x_b, y_b in dl:
            ...
            h0 = torch.zeros(model.lstm.num_layers, x_b.size(0),
...
# Training mit Mixed Precision, Validierung & Batch-Fenstern
def train_online(epochs=20, lr=1e-3, seq_len=50, batch_size=64):
    train_scaled, df_val, df_test, train_cells, val_cell = load_data()
    print("Training auf Zellen:", train_cells)
    print("Validierung auf Zelle:", val_cell)

    # Rohdaten-Plots (optional)
    ...
    plt.plot(df_val['timestamp'], df_val['SOC_ZHU']); plt.title("Val SOC")
    ...
    for ep in range(1, epochs+1):
        ...
        # --- VALIDIERUNG ---
        val_loss = evaluate(model, df_val, seq_len, batch_size)

        # --- AUSGABE & SCHRITTE STEUERN ---
        ...
        if val_loss < best_val_loss:
            ...
            if patience_counter >= 5:
                print("Early Stopping aktiviert (keine Verbesserung im Val-Loss)")
                break

    # --- FINALER TEST ---
    print("\nStarte finalen Test auf Zelle:", val_cell)
    test_online()  # lädt 'best_online_soc.pth' und wertet df_test aus
